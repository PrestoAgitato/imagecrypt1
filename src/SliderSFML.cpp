...
void SliderSFML::setCallback(const std::function<void(float)>& callback) {
...
void SliderSFML::setFont(sf::Font& font) {
	text.setFont(font);
}
...
void SliderSFML::logic(sf::RenderWindow& window)
{
		...
		&& sf::Mouse::isButtonPressed(sf::Mouse::Button::Left))
	{
		if (sf::Mouse::getPosition(window).x >= xCord && sf::Mouse::getPosition(window).x <= xCord + axisWidth)
		{
			...
			if (callbackFunction && oldValue != sliderValue) {
				callbackFunction(sliderValue);
			}
		}
	}
}
